...
def station_priority(path):
    ...
    return len(subway)
def station_priority1(path):
    subway=[]
    for station in path:
        subway.append(station.split("-")[1])
    subway=list(set(subway))
    return len(subway)+len(path)
...
def gather_first(pathes):
    ways = {}
    res = []
    if len(pathes) <= 1: return pathes
    for path in pathes:
        num = station_priority1(path)
        if num not in ways:
            ways[num] = []
        ways[num].append(path)
    sorway = sorted(ways.items(), key=lambda m: m[0])
...
if __name__=="__main__":
    ...
    print(des1)
    #综合优先 将换成的地铁数和总的站点数加起来，最小的优先
    des2=search1('中关村站-北京地铁4号线', '平安里站', simple_connection_info, sort_candidate=gather_first)
    print(des2)